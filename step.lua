...
function W.Attack(target) pcall(function() _G.FastAttack = os.time() end) end
    ...
    FunctionsHandler.BossesTask:RegisterMethod('Start', function(k)
        if k then
            ...
            CombatController.Attack(tostring(k), nil, nil, function() SpecialItems = nil end)
            SpecialItems = nil
        end
    end)
    ...
    FunctionsHandler.AutoRaidIce:RegisterMethod("BuyChip", function()
        ...
        if fruitName then
            SetTask('MainTask', 'Auto Raid Ice | Buy chip with fruit ' .. fruitName)
            ...
            if CheckSpecialMicrochip() then
                ...
                SetTask('MainTask', 'Auto Raid Ice | Buy chip failed')
                return false
            end
        elseif (ScriptStorage.PlayerData.Beli or 0) >= 100000 then
            SetTask('MainTask', 'Auto Raid Ice | Buy chip with Beli')
            Remotes.CommF_:InvokeServer("RaidsNpc", "Select", "Ice")
            task.wait(1)
            RefreshInventory()
            if CheckSpecialMicrochip() then
        ...
        SetTask('MainTask', 'Auto Raid Ice | Not enough resources to buy chip')
        return false
    end)
    ...
    FunctionsHandler.AutoRaidIce:RegisterMethod("Refresh", function()
        ...
        if lv < 1300 then return nil end
        if fr >= target then
            SetTask('MainTask', 'Raid Ice | Target reached: ' .. fr .. '/' .. target)
            return nil
        end
    ...
    FunctionsHandler.AutoRaidIce:RegisterMethod("Start", function()
        ...
        local fr = ScriptStorage.PlayerData.Fragments or 0
        if fr >= target then
            SetTask('MainTask', 'Raid Ice | Target fragments reached')
            return
        end
        if currentIsland then
            local islandNum = tonumber(string.match(currentIsland.Name, "(%d+)"))
            SetTask('MainTask', 'Raid Ice | Fragments: ' .. fr .. '/' .. target .. ' | Island ' .. (islandNum or "?") .. '/5')
            SetTask('SubTask', 'Raid Ice | Clearing island ' .. (islandNum or "?"))
            if islandNum and islandNum >= 3 then
                SetTask('MainTask', 'Raid Ice | Island ' .. islandNum .. ' | Kill Aura')
        ...
        if not mapObj then
            Report('[AutoRaidIce] Map not found: ' .. mapName)
            return
        end
        local summonCFrame
        ...
        if summonCFrame then
            SetTask('MainTask', 'Raid Ice | Go to summon button')
        ...
        for retry = 1, 3 do
            ...
            pcall(function()
                if btn and btn.RaidSummon2 and btn.RaidSummon2.Button and btn.RaidSummon2.Button.Main and btn.RaidSummon2.Button.Main.ClickDetector then
                    ...
                    clickSuccess = true
                    SetTask('MainTask', 'Raid Ice | Clicked start (attempt ' .. retry .. ')')
                else
                    SetTask('MainTask', 'Raid Ice | Button not found (attempt ' .. retry .. ')')
                end
            end)
            if clickSuccess then break end
        end
        if not clickSuccess then
            SetTask('MainTask', 'Raid Ice | Cannot click start')
            return
        end
        SetTask('MainTask', 'Raid Ice | Waiting for raid to start...')
        local raidStarted = false
        for attempt = 1, 3 do
            if attempt > 1 then
                SetTask('MainTask', 'Raid Ice | Retry attempt ' .. attempt)
                pcall(function()
                    if btn and btn.RaidSummon2 and btn.RaidSummon2.Button and btn.RaidSummon2.Button.Main and btn.RaidSummon2.Button.Main.ClickDetector then
                        fireclickdetector(btn.RaidSummon2.Button.Main.ClickDetector)
                    end
        ...
        if raidStarted then
            LastRaidAlert = 0
            SetTask('MainTask', 'Raid Ice | Raid started!')
        else
            Report('[AutoRaidIce] Raid did not start')
            SetTask('MainTask', 'Raid Ice | Raid did not start, retrying later...')
        end
    end)
    ...
    -- ============================================================
    -- UTILITY ITEMS ACTIVATION
    -- ============================================================
    FunctionsHandler.UtilityItemsActivation:RegisterMethod('Refresh', function()
        ...
        for k, k in SpecialItems do
            if ScriptStorage.Tools[k] then
                FunctionsHandler.UtilityItemsActivation:Set('CurrentProgressLevel', k)
                return k
            end
        end
        if SeaIndex == 3 and (ScriptStorage.Melees["Death Step"] or 0) >= 400 and (ScriptStorage.Melees["Black Leg"] or 0) >= 400 and ScriptStorage.PlayerData.Beli >= 2500000 and ScriptStorage.PlayerData.Fragments >= 5000 and not ScriptStorage.Melees['Electric Claw'] then
            FunctionsHandler.UtilityItemsActivation:Set('CurrentProgressLevel', "Previous Hero")
            return 'Previous Hero'
        end
        if ScriptStorage.Tools["Red Key"] then
            FunctionsHandler.UtilityItemsActivation:Set("CurrentProgressLevel", "Red Key")
            return 'Red Key'
        end
        if ScriptStorage.Tools['Hallow Essence'] then
            FunctionsHandler.UtilityItemsActivation:Set("CurrentProgressLevel", 'Soul Reaper Spawner')
        ...
        if ScriptStorage.Tools['Fire Essence'] then
            FunctionsHandler.UtilityItemsActivation:Set('CurrentProgressLevel', "Uzoth")
            return 'Uzoth'
        end
    end)

    FunctionsHandler.UtilityItemsActivation:RegisterMethod('Start', function()
        local k = FunctionsHandler.UtilityItemsActivation:Get("CurrentProgressLevel")
        ...
        elseif k == "Red Key" then
            ...
            alert('Red Key', "Submitting red key to the scientist.")
    ...
    local function _SgHopServer()
        SetTask("MainTask", "🎸 Soul Guitar | Hop: Searching for server...")
        ...
        SetTask("MainTask", "🎸 Soul Guitar | Hop: No server found, retrying...")
        task.wait(5)
    end
    ...
    local function _SgRunChestBatch()
        ...
        if hasItem then
            SetTask("MainTask", "🎸 Auto Chest: Found " .. itemName .. " — stop")
            return false
        end
        ...
        SetTask("SubTask", "🎸 Auto Chest: " .. #chests .. " chests remaining")
        if #chests == 0 then
            return false
        end
    ...
    FunctionsHandler.SoulGuitar:RegisterMethod("Refresh", function()
        ...
        if not ScriptStorage.Backpack['Dark Fragment'] then            if ScriptStorage.Backpack['Fist of Darkness'] then
                return 10
            end
            if not _SgChestPhaseDone then
                ...
                if #chests > 0 then
                    ...
                    SetTask("MainTask", "🎸 Soul Guitar | No more chests, hopping for Fist of Darkness")
    ...
    FunctionsHandler.SoulGuitar:RegisterMethod('Start', function(k)
        if k == 9 then
            SetTask("MainTask", "🎸 Soul Guitar | Auto Chest: Collecting...")
        ...
        elseif k == 10 then
            ...
            task.wait(1)
            SetTask('MainTask', '🎸 Soul Guitar: Defeating Blackbeard for Dark Fragment')
        ...
        elseif k == 1 then
            if SeaIndex ~= 2 then
                SetTask("SubTask", '🎸 Soul Guitar: Teleport to Sea 2 to farm ectoplasm')
    ...
    if Config.Configuration and Config.Configuration.LowGraphics ~= false then
        task.spawn(function()
            pcall(function()
                ...
                print("[✅ Anti Lag] Applied graphics optimization!")
            end)
        end)
    end
